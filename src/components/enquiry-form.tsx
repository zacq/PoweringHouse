...
export function EnquiryForm({ offerId, offerName }: { offerId: string; offerName: string }) {
  ...
  if (status === "ok") {
    return <p className="comment-form__note" data-state="ok">{message}</p>;
  }

  return (
    <form className="comment-form" onSubmit={onSubmit} aria-label={`Enquire about ${offerName}`}>
      ...
      <div className="comment-form__row">
        ...
        <div>
          ...
          <input id="businessName" name="businessName" type="text" maxLength={160} />
        </div>
      </div>
      <div>
